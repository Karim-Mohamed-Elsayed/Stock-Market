...
def get_last_timestamp(ticker: str) -> str | None:
    ...
    object_key = f"hourly/{ticker}.csv"  # ◄ Added 'hourly/' folder prefix
    try:
        ...
        # Key does not exist yet (first execution inside this folder)
...
def append_or_create(ticker: str, new_data: pd.DataFrame) -> None:
    ...
    try:
        # Check for existing records inside the target AWS S3 bucket
        response = s3_client.get_object(Bucket=BUCKET_NAME, Key=object_key)
