"""
S&P 500 Hourly Data Downloader
- First run : downloads last 2 years of hourly data
- Daily runs: checks last stored timestamp and appends only missing hours
"""
...
def get_last_timestamp(ticker: str) -> str | None:
    """Return the next fetch start (1 hour after last stored row), or None."""
...
def main():
    ...
    if all_failed:
        ...
        log.warning(f"{len(all_failed)} failed tickers saved to failed_hourly.csv")
