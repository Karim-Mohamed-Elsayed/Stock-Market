...
export default function InsightsAndAnalyticsPage() {
  ...
  if (loadingInsights) {
    return (
      <main style={{ paddingTop: '100px', minHeight: '80vh', padding: '100px 40px 0 40px' }}>
  ...
  return (
    <main style={{ paddingTop: '64px', minHeight: '80vh', paddingBottom: '64px', paddingLeft: '40px', paddingRight: '40px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '24px' }}>
        ...
        <div style={{ background: 'var(--bg-2)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: '24px' }}>
          <h2 style={{ fontSize: '20px', fontWeight: 600, marginBottom: '24px' }}>Sector Performance Ranking</h2>
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', textAlign: 'left', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid var(--border)', color: 'var(--text-tertiary)', fontSize: '12px', textTransform: 'uppercase' }}>
                  <th style={{ padding: '12px 8px' }}>Rank</th>
                  <th style={{ padding: '12px 8px' }}>GICS Sector</th>
                  <th style={{ padding: '12px 8px', textAlign: 'right' }}>Avg Daily Return</th>
                </tr>
              </thead>
              <tbody>
                {sectors.map((sector, idx) => {
                  const isGain = sector.avg_daily_return >= 0;
                  const isExpanded = expandedSectors[sector.gics_sector];
                  ...
                  return (
                    <React.Fragment key={sector.gics_sector}>
                      <tr 
                        style={{ borderBottom: isExpanded ? 'none' : '1px solid var(--border)', fontSize: '14px', cursor: 'pointer', transition: 'background 0.2s' }}
                        onClick={() => setExpandedSectors(prev => ({...prev, [sector.gics_sector]: !prev[sector.gics_sector]}))}
                        onMouseEnter={(e) => e.currentTarget.style.background = 'var(--bg-3)'}
                        onMouseLeave={(e) => e.currentTarget.style.background = 'transparent'}
                      >
                        <td style={{ padding: '16px 8px', color: 'var(--text-secondary)' }}>#{idx + 1}</td>
                        <td style={{ padding: '16px 8px', fontWeight: 500, display: 'flex', alignItems: 'center', gap: '12px' }}>
                          <span style={{ 
                        ...
                        <td style={{ padding: '16px 8px', textAlign: 'right', fontWeight: 600, color: isGain ? 'var(--gain)' : 'var(--loss)' }}>
        ...
        <div style={{ background: 'var(--bg-2)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: '24px', display: 'flex', flexDirection: 'column' }}>
          <h2 style={{ fontSize: '20px', fontWeight: 600, marginBottom: '24px', flexShrink: 0 }}>Ticker RSI Rankings</h2>
          <div style={{ overflowX: 'auto', overflowY: 'auto', flex: 1, minHeight: 0, maxHeight: '700px' }}>
            <table style={{ width: '100%', textAlign: 'left', borderCollapse: 'collapse' }}>
              <thead style={{ position: 'sticky', top: 0, background: 'var(--bg-2)' }}>
                <tr style={{ borderBottom: '1px solid var(--border)', color: 'var(--text-tertiary)', fontSize: '12px', textTransform: 'uppercase' }}>
                  <th style={{ padding: '12px 8px' }}>Rank</th>
                  <th style={{ padding: '12px 8px' }}>Ticker</th>
                  <th style={{ padding: '12px 8px', textAlign: 'right' }}>RSI (14)</th>
              ...
              <tbody>
                {quotes.map((quote, idx) => {
                  ...
                  return (
                    <tr key={quote.ticker} style={{ borderBottom: '1px solid var(--border)', fontSize: '14px' }}>
                      <td style={{ padding: '16px 8px', color: 'var(--text-secondary)' }}>#{idx + 1}</td>
                      <td style={{ padding: '16px 8px' }}>
                        <Link href={`/markets/${quote.ticker}`} style={{ display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--text-primary)', textDecoration: 'none' }}>
                          <TickerLogo ticker={quote.ticker} size={20} />
                          <span style={{ fontWeight: 600 }}>{quote.ticker}</span>
                        </Link>
                      </td>
                      <td style={{ padding: '16px 8px', textAlign: 'right', fontWeight: 500, fontFamily: 'var(--font-mono)' }}>
                        {rsi.toFixed(2)}
                      </td>
                      <td style={{ padding: '16px 8px', color: statusColor, fontWeight: 500 }}>
                        {status}
                      </td>
                    </tr>
                  );
        ...
        {/* Tickers Volatility Table */}
        <div style={{ background: 'var(--bg-2)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: '24px', display: 'flex', flexDirection: 'column' }}>
          <h2 style={{ fontSize: '20px', fontWeight: 600, marginBottom: '24px', flexShrink: 0 }}>Highest Volatility Tickers</h2>
          <div style={{ overflowX: 'auto', overflowY: 'auto', flex: 1, minHeight: 0, maxHeight: '700px' }}>
            <table style={{ width: '100%', textAlign: 'left', borderCollapse: 'collapse' }}>
              <thead style={{ position: 'sticky', top: 0, background: 'var(--bg-2)' }}>
                <tr style={{ borderBottom: '1px solid var(--border)', color: 'var(--text-tertiary)', fontSize: '12px', textTransform: 'uppercase' }}>
                  <th style={{ padding: '12px 8px' }}>Rank</th>
                  <th style={{ padding: '12px 8px' }}>Ticker</th>
                  <th style={{ padding: '12px 8px', textAlign: 'right' }}>30d Volatility</th>
                </tr>
              </thead>
              <tbody>
                {volatilityQuotes.map((quote, idx) => {
                ...
                {volatilityQuotes.length === 0 && (
                  <tr>
                    <td colSpan={3} style={{ textAlign: 'center', padding: '32px', color: 'var(--text-tertiary)' }}>No Volatility data available.</td>
                  </tr>
                )}
              </tbody>
            </table>
      ...
      <div style={{ marginTop: '80px', paddingTop: '64px', borderTop: '1px solid var(--border)' }}>
